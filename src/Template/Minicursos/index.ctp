...
<?= $this->Flash->render('auth') ?>
    ...
    <?php if (strpos('admin', $loguser ['role']) !== false) {?>
    	<?= $this->Html->link('<i class="fa fa-2x fa-fw fa-plus pull-left"> Novo Minicurso</i> ' , array ('controller' => 'minicursos', 'action' => 'add' ), array ('class'=>'btn btn-success btn-sm col-xs-3','escape' => false) )?>
    <?php }?>
    <table cellpadding="0" cellspacing="0">
		...
		if (strpos('admin', $loguser ['role']) !== false) {?>
			...
			<?=$this->Form->postLink ('<i class="fa fa-lg fa-fw fa-trash"></i>' , array ('action' => 'delete',$minicurso->id ), array ('class' => 'btn btn-danger' ,'escape' => false,'confirm' => __ ( 'Você tem certeza que deseja remover permanentemente o minicurso Nº' . $minicurso->id . ' ?' ) ) );?>
			<?php }?>
